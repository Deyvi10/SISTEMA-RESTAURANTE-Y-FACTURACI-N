...
- **`impresoras`**: `id`, `local_id`, `nombre`, `conexion ENUM(TCP, USB, WINDOWS)`, `host`, `puerto`, `mac`, `usb_id`, `nombre_windows` (cola del spooler de la PC del nodo), `modelo`, `ancho_papel ENUM(58,80)`, `origen ENUM(MANUAL, DETECTADA)`, `activa`. El hardware lo informa el nodo (evento `impresora.detectada`) y el nombre, el ancho y las estaciones los decide el dueño. El estado vivo no se guarda aquí: viaja en el heartbeat del nodo.
- **`comandos_nodo`**: órdenes de la nube para el nodo de un local (`IMPRIMIR_PRUEBA`, `BUSCAR_IMPRESORAS`). Viajan por el feed de cambios y el nodo responde con el evento `comando.ejecutado`.
- **`estacion_impresoras`**: N:M (una estación puede imprimir en dos impresoras).
- **`trabajos_impresion`** (solo en el nodo): `id`, `impresora_id`, `estacion_id`, `comanda_id`, `comando_id`, `tipo ENUM(COMANDA, ANULACION, REIMPRESION, PRECUENTA, PRUEBA)`, `payload BLOB`, `documento JSON` (para volver a armar el ticket si se redirige), `estado ENUM(PENDIENTE, IMPRESO, CANCELADO)`, `intentos`, `ultimo_error`. Lo pendiente nunca se borra; solo se cancelan las pruebas vencidas.
- **`redirecciones`** (solo en el nodo): `estacion_id` → `impresora_id` temporal (RF-02-05.3). **`anulaciones_comanda`** (append-only).
