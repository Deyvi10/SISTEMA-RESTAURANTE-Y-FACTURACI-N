...
func Load() (Config, error) {
	...
	required := map[string]string{
		"DATABASE_URL": c.DatabaseURL, "PUBLIC_URL": c.PublicURL, "BACKOFFICE_URL": c.BackofficeURL, "SMTP_ADDR": c.SMTPAddr,
	}
	switch c.StorageDriver {
	case "s3":
		required["S3_ENDPOINT"], required["S3_ACCESS_KEY"], required["S3_SECRET_KEY"] = c.S3Endpoint, c.S3AccessKey, c.S3SecretKey
	case "azure":
		required["AZURE_STORAGE_CONNECTION_STRING"] = c.AzureStorageConn
	default:
		return c, fmt.Errorf("config: STORAGE_DRIVER debe ser s3 o azure, no %q", c.StorageDriver)
	}
