    ...
    return CupertinoPageScaffold(
      child: SafeArea(
        ...
        child: salon.when(
          ...
          error: (e, _) => Center(
            child: Text('No se pudo cargar el salón.', style: RpText.body.copyWith(color: c.labelSecondary)),
          ),
          data: (s) {
            ...
            return CustomScrollView(
              slivers: [
                ...
                if (pendientes.isNotEmpty) SliverToBoxAdapter(child: _BannerPendientes(n: pendientes.length)),
                if (s.zonas.length > 1)
                  SliverToBoxAdapter(
                    child: Padding(
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              '$n ${n == 1 ? 'comanda pendiente' : 'comandas pendientes'} de envío. Se enviarán solas al volver la conexión.',
...
class _Leyenda extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget chip(String t, Color col) => Row(
            ...
            children: [
              Positioned.fill(
                child: Column(
                  ...
                  children: [
                    Text(
                      mesa.nombre.replaceAll(numero, '').trim().isEmpty ? 'Mesa' : mesa.nombre.replaceAll(numero, '').trim(),
                    ...
                    Expanded(
                      child: FittedBox(
                        ...
                        child: Text(numero, style: RpText.amount.copyWith(color: col.fg)),
                      ),
                    ),
                    Text(
                      detalle,
                      ...
                      style: RpText.caption1.copyWith(color: col.fg, fontWeight: FontWeight.w600),
                    ),
                    if (mesa.meseroNombre != null && !ajena)
                      Text(
                        ...
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: RpText.caption2.copyWith(color: col.fg.withValues(alpha: .8)),
                      ),
                  ],
                ),
              ),
